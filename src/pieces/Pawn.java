...
public class Pawn extends Piece {
    public Pawn(Board board, int col, int row, boolean isWhite) {
        ...
        this.sprite = sheet.getSubimage(5 * sheetScale, isWhite ? 0 : sheetScale, sheetScale, sheetScale).getScaledInstance(board.titeSize, board.titeSize, BufferedImage.SCALE_SMOOTH);
    }
    ...
    public boolean isValidMovement(int col, int row) {
        ...
        // bắt tốt qua đường
        // en passant left
        if (board.getTileNum(col, row) == board.enPassantTitle && col == this.col - 1 && row == this.row - colorIndex && board.getPiece(col, row + colorIndex) != null) {
            return true;
        }
        // en passant right
        if (board.getTileNum(col, row) == board.enPassantTitle && col == this.col + 1 && row == this.row - colorIndex && board.getPiece(col, row + colorIndex) != null) {
